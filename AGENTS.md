...
Use the repository-local development launcher (`scripts/dev.sh`) for relevant behavioral smoke tests so changes are exercised in the current checkout instead of relying solely on the automated test matrix. Follow `docs/development.md` for supported commands, isolation boundaries, and wake-listener safety.
